...
Эта база данных автоматически синхронизируется с актуальным кодом PLC (`FB_ModbusToSCADA.st`) и предоставляет удобные инструменты для:
...
├── migrate_from_fc.py              # Парсер FB_ModbusToSCADA.st → DB
...
📄 Парсинг файла: FB_ModbusToSCADA.st
   Найдено секций: 21
...
1. Добавить вызов `FC_ModbusRead/Write` в `POUs/FB_ModbusToSCADA.st`
...
```bash
python3 migrate_from_fc.py    # Пересоздаст БД из FB_ModbusToSCADA.st
